...
test.describe('Smoke Test', () => {
  test('should load the page without errors', async ({ page }) => {
    ...
    await expect(page.locator('h1:has-text("Takazudo Modular Case Estimate")')).toBeVisible();
  ...
  test('should have interactive elements working', async ({ page }) => {
    ...
    // Check that rail type buttons are present
    const railButtons = page.locator(
      'button:has-text("Lite"), button:has-text("Dual"), button:has-text("Metal")',
    );
    await expect(railButtons.first()).toBeVisible();

    // Check that panel list is present
    await expect(page.locator('text=Panels')).toBeVisible();

    // Check that price display is present
