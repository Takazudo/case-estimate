      ...
      <ul className="flex flex-col gap-vgap-xs border-t border-zd-gray border-dashed pt-vgap-md">
        {items.map((item) => (
          <li key={item.href}>
            <Link
              href={item.href}
              className="inline-flex items-center gap-hgap-xs group no-underline zd-invert-color-link"
            >
              <span className="inline-flex items-center gap-[6px] px-hgap-xs py-vgap-xs rounded text-sm md:text-base text-zd-white whitespace-nowrap zd-button-gradient transition-all">
                <svg
