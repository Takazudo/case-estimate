...
export default function CategoryNav({ category }: CategoryNavProps): ReactNode {
  ...
  if (!data) {
    return <p className={styles.empty}>カテゴリが見つかりません。</p>;
  }
  ...
  if (!hasContent) {
    return <p className={styles.empty}>このカテゴリにはまだページがありません。</p>;
  }
    ...
    <ul className={styles.navList}>
      ...
      {subcategories.map((subcat) => (
        <li key={subcat.key} className={styles.navItem}>
          {subcat.hasIndex && subcat.docId ? (
            <a href={`${docsBaseUrl}${subcat.docId}`}>{subcat.title}</a>
          ) : (
            <span className={styles.subcategoryLabel}>{subcat.title}</span>
          )}
