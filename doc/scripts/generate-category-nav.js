...
// Set to 'auto' to auto-detect subdirectories, or provide an array of specific subcategory names
 ...
 */
function findSubdirectories(dirPath) {
...
/**
 * Get category label from _category_.json if it exists
 */
function getCategoryLabel(dirPath) {
  ...
  try {
    const content = fs.readFileSync(categoryJsonPath, 'utf8');
...
/**
 * Get category position from _category_.json if it exists
 */
...
function main() {
  ...
  Object.entries(CATEGORY_STRUCTURE).forEach(([category, subcategoriesConfig]) => {
    const categoryDir = path.join(DOCS_DIR, category);

    // Skip if category directory doesn't exist
    if (!fs.existsSync(categoryDir)) {
      console.log(`  ${category}: Directory not found, creating empty entry`);
    ...
    // Resolve subcategories: 'auto' means auto-detect, array means explicit list
    const subcategories =
      subcategoriesConfig === 'auto' ? findSubdirectories(categoryDir) : subcategoriesConfig;

    const subcats = subcategories
      .map((subcat) => {
        const subcatDir = path.join(categoryDir, subcat);
        const indexInfo = getIndexInfo(subcatDir);
        const categoryLabel = getCategoryLabel(subcatDir);
        const categoryPosition = getCategoryPosition(subcatDir);
        ...
        return {
          key: subcat,
          title: indexInfo?.title || categoryLabel || subcat,
          docId: indexInfo?.docId || null,
          hasIndex: !!indexInfo,
          position: indexInfo?.position ?? categoryPosition ?? 999,
          pages: subcatPages,
        };
      })
      .filter(Boolean)
