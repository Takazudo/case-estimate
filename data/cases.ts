import type { Cases } from '@/types';

export type CaseEntry = [string, Cases[keyof Cases]];

export const cases: Cases = {
  'zudo-block-40-3DP-A': {
    name: 'zudo-block-40-3DP-A',
    hp: 40,
    material: '3dp',
    panels: [
      { id: 'side1', name: 'サイド1' },
      { id: 'side2', name: 'サイド2' },
      { id: 'back1', name: 'バック1' },
      { id: 'back2', name: 'バック2' },
      { id: 'bottom1', name: 'ボトム1' },
      { id: 'bottom2', name: 'ボトム2' },
      { id: 'front1', name: 'フロント1' },
      { id: 'front2', name: 'フロント2' },
    ],
  },
  'zudo-block-40-3DP-B': {
    name: 'zudo-block-40-3DP-B',
    hp: 40,
    material: '3dp',
    panels: [
      { id: 'side1', name: 'サイド1' },
      { id: 'side2', name: 'サイド2' },
      { id: 'back1', name: 'バック1' },
      { id: 'back2', name: 'バック2' },
      { id: 'bottom1', name: 'ボトム1' },
      { id: 'bottom2', name: 'ボトム2' },
      { id: 'front1', name: 'フロント1' },
      { id: 'front2', name: 'フロント2' },
    ],
  },
  'zudo-block-40-ACR-A': {
    name: 'zudo-block-40-ACR-A',
    hp: 40,
    material: 'acrylic',
    panels: [
      { id: 'side1', name: 'サイド1' },
      { id: 'side2', name: 'サイド2' },
      { id: 'back1', name: 'バック1' },
      { id: 'back2', name: 'バック2' },
      { id: 'bottom1', name: 'ボトム1' },
      { id: 'bottom2', name: 'ボトム2' },
      { id: 'front1', name: 'フロント1' },
      { id: 'front2', name: 'フロント2' },
    ],
  },
  'zudo-block-40-ACR-B': {
    name: 'zudo-block-40-ACR-B',
    hp: 40,
    material: 'acrylic',
    panels: [
      { id: 'side1', name: 'サイド1' },
      { id: 'side2', name: 'サイド2' },
      { id: 'back1', name: 'バック1' },
      { id: 'back2', name: 'バック2' },
      { id: 'bottom1', name: 'ボトム1' },
      { id: 'bottom2', name: 'ボトム2' },
      { id: 'front1', name: 'フロント1' },
      { id: 'front2', name: 'フロント2' },
    ],
  },
  'zudo-block-40x2-3DP-A': {
    name: 'zudo-block-40x2-3DP-A',
    hp: 80,
    material: '3dp',
    panels: [
      { id: 'side1', name: 'サイド1' },
      { id: 'side2', name: 'サイド2' },
      { id: 'side3', name: 'サイド3' },
      { id: 'side4', name: 'サイド4' },
      { id: 'back1', name: 'バック1' },
      { id: 'back2', name: 'バック2' },
      { id: 'bottom1', name: 'ボトム1' },
      { id: 'bottom2', name: 'ボトム2' },
      { id: 'bottom3', name: 'ボトム3' },
      { id: 'bottom4', name: 'ボトム4' },
      { id: 'front1', name: 'フロント1' },
      { id: 'front2', name: 'フロント2' },
    ],
  },
  'zudo-block-40x2-3DP-B': {
    name: 'zudo-block-40x2-3DP-B',
    hp: 80,
    material: '3dp',
    panels: [
      { id: 'side1', name: 'サイド1' },
      { id: 'side2', name: 'サイド2' },
      { id: 'side3', name: 'サイド3' },
      { id: 'side4', name: 'サイド4' },
      { id: 'back1', name: 'バック1' },
      { id: 'back2', name: 'バック2' },
      { id: 'bottom1', name: 'ボトム1' },
      { id: 'bottom2', name: 'ボトム2' },
      { id: 'bottom3', name: 'ボトム3' },
      { id: 'bottom4', name: 'ボトム4' },
      { id: 'front1', name: 'フロント1' },
      { id: 'front2', name: 'フロント2' },
    ],
  },
  'zudo-block-40x2-ACR-A': {
    name: 'zudo-block-40x2-ACR-A',
    hp: 80,
    material: 'acrylic',
    panels: [
      { id: 'side1', name: 'サイド1' },
      { id: 'side2', name: 'サイド2' },
      { id: 'side3', name: 'サイド3' },
      { id: 'side4', name: 'サイド4' },
      { id: 'back1', name: 'バック1' },
      { id: 'back2', name: 'バック2' },
      { id: 'bottom1', name: 'ボトム1' },
      { id: 'bottom2', name: 'ボトム2' },
      { id: 'bottom3', name: 'ボトム3' },
      { id: 'bottom4', name: 'ボトム4' },
      { id: 'front1', name: 'フロント1' },
      { id: 'front2', name: 'フロント2' },
    ],
  },
  'zudo-block-40x2-ACR-B': {
    name: 'zudo-block-40x2-ACR-B',
    hp: 80,
    material: 'acrylic',
    panels: [
      { id: 'side1', name: 'サイド1' },
      { id: 'side2', name: 'サイド2' },
      { id: 'side3', name: 'サイド3' },
      { id: 'side4', name: 'サイド4' },
      { id: 'back1', name: 'バック1' },
      { id: 'back2', name: 'バック2' },
      { id: 'bottom1', name: 'ボトム1' },
      { id: 'bottom2', name: 'ボトム2' },
      { id: 'bottom3', name: 'ボトム3' },
      { id: 'bottom4', name: 'ボトム4' },
      { id: 'front1', name: 'フロント1' },
      { id: 'front2', name: 'フロント2' },
    ],
  },
  'zudo-block-60-3DP-A': {
    name: 'zudo-block-60-3DP-A',
    hp: 60,
    material: '3dp',
    panels: [
      { id: 'side1', name: 'サイド1' },
      { id: 'side2', name: 'サイド2' },
      { id: 'back1', name: 'バック1' },
      { id: 'back2', name: 'バック2' },
      { id: 'bottom1', name: 'ボトム1' },
      { id: 'bottom2', name: 'ボトム2' },
      { id: 'front1', name: 'フロント1' },
      { id: 'front2', name: 'フロント2' },
    ],
  },
  'zudo-block-60-3DP-B': {
    name: 'zudo-block-60-3DP-B',
    hp: 60,
    material: '3dp',
    panels: [
      { id: 'side1', name: 'サイド1' },
      { id: 'side2', name: 'サイド2' },
      { id: 'back1', name: 'バック1' },
      { id: 'back2', name: 'バック2' },
      { id: 'bottom1', name: 'ボトム1' },
      { id: 'bottom2', name: 'ボトム2' },
      { id: 'front1', name: 'フロント1' },
      { id: 'front2', name: 'フロント2' },
    ],
  },
  'zudo-block-60-ACR-A': {
    name: 'zudo-block-60-ACR-A',
    hp: 60,
    material: 'acrylic',
    panels: [
      { id: 'side1', name: 'サイド1' },
      { id: 'side2', name: 'サイド2' },
      { id: 'back1', name: 'バック1' },
      { id: 'back2', name: 'バック2' },
      { id: 'bottom1', name: 'ボトム1' },
      { id: 'bottom2', name: 'ボトム2' },
      { id: 'front1', name: 'フロント1' },
      { id: 'front2', name: 'フロント2' },
    ],
  },
  'zudo-block-60-ACR-B': {
    name: 'zudo-block-60-ACR-B',
    hp: 60,
    material: 'acrylic',
    panels: [
      { id: 'side1', name: 'サイド1' },
      { id: 'side2', name: 'サイド2' },
      { id: 'back1', name: 'バック1' },
      { id: 'back2', name: 'バック2' },
      { id: 'bottom1', name: 'ボトム1' },
      { id: 'bottom2', name: 'ボトム2' },
      { id: 'front1', name: 'フロント1' },
      { id: 'front2', name: 'フロント2' },
    ],
  },
  'zudo-block-60x2-3DP-A': {
    name: 'zudo-block-60x2-3DP-A',
    hp: 120,
    material: '3dp',
    panels: [
      { id: 'side1', name: 'サイド1' },
      { id: 'side2', name: 'サイド2' },
      { id: 'side3', name: 'サイド3' },
      { id: 'side4', name: 'サイド4' },
      { id: 'back1', name: 'バック1' },
      { id: 'back2', name: 'バック2' },
      { id: 'bottom1', name: 'ボトム1' },
      { id: 'bottom2', name: 'ボトム2' },
      { id: 'bottom3', name: 'ボトム3' },
      { id: 'bottom4', name: 'ボトム4' },
      { id: 'front1', name: 'フロント1' },
      { id: 'front2', name: 'フロント2' },
    ],
  },
  'zudo-block-60x2-3DP-B': {
    name: 'zudo-block-60x2-3DP-B',
    hp: 120,
    material: '3dp',
    panels: [
      { id: 'side1', name: 'サイド1' },
      { id: 'side2', name: 'サイド2' },
      { id: 'side3', name: 'サイド3' },
      { id: 'side4', name: 'サイド4' },
      { id: 'back1', name: 'バック1' },
      { id: 'back2', name: 'バック2' },
      { id: 'bottom1', name: 'ボトム1' },
      { id: 'bottom2', name: 'ボトム2' },
      { id: 'bottom3', name: 'ボトム3' },
      { id: 'bottom4', name: 'ボトム4' },
      { id: 'front1', name: 'フロント1' },
      { id: 'front2', name: 'フロント2' },
    ],
  },
  'zudo-block-60x2-ACR-A': {
    name: 'zudo-block-60x2-ACR-A',
    hp: 120,
    material: 'acrylic',
    panels: [
      { id: 'side1', name: 'サイド1' },
      { id: 'side2', name: 'サイド2' },
      { id: 'side3', name: 'サイド3' },
      { id: 'side4', name: 'サイド4' },
      { id: 'back1', name: 'バック1' },
      { id: 'back2', name: 'バック2' },
      { id: 'bottom1', name: 'ボトム1' },
      { id: 'bottom2', name: 'ボトム2' },
      { id: 'bottom3', name: 'ボトム3' },
      { id: 'bottom4', name: 'ボトム4' },
      { id: 'front1', name: 'フロント1' },
      { id: 'front2', name: 'フロント2' },
    ],
  },
  'zudo-block-60x2-ACR-B': {
    name: 'zudo-block-60x2-ACR-B',
    hp: 120,
    material: 'acrylic',
    panels: [
      { id: 'side1', name: 'サイド1' },
      { id: 'side2', name: 'サイド2' },
      { id: 'side3', name: 'サイド3' },
      { id: 'side4', name: 'サイド4' },
      { id: 'back1', name: 'バック1' },
      { id: 'back2', name: 'バック2' },
      { id: 'bottom1', name: 'ボトム1' },
      { id: 'bottom2', name: 'ボトム2' },
      { id: 'bottom3', name: 'ボトム3' },
      { id: 'bottom4', name: 'ボトム4' },
      { id: 'front1', name: 'フロント1' },
      { id: 'front2', name: 'フロント2' },
    ],
  },
  '10box-shallow-3dp': {
    name: '10BOX-shallow-3DP',
    hp: 104,
    material: '3dp',
    panels: [
      { id: 'main-side1', name: 'メイン: サイド1' },
      { id: 'main-side2', name: 'メイン: サイド2' },
      { id: 'main-back1', name: 'メイン: バック1' },
      { id: 'main-bottom1', name: 'メイン: ボトム1' },
      { id: 'main-bottom2', name: 'メイン: ボトム2' },
      { id: 'main-front', name: 'メイン: フロント' },
      { id: 'main-side3', name: 'メイン: サイド3' },
      { id: 'main-side4', name: 'メイン: サイド4' },
      { id: 'lid-side1', name: 'フタ: サイド1' },
      { id: 'lid-back', name: 'フタ: バック' },
      { id: 'lid-top1', name: 'フタ: トップ1' },
      { id: 'lid-top2', name: 'フタ: トップ2' },
      { id: 'lid-front', name: 'フタ: フロント' },
      { id: 'lid-side2', name: 'フタ: サイド2' },
      { id: 'stand-angle1', name: 'スタンド: アングル1' },
      { id: 'stand-angle2', name: 'スタンド: アングル2' },
      { id: 'stand-support1', name: 'スタンド: サポート1' },
      { id: 'stand-support2', name: 'スタンド: サポート2' },
    ],
  },
  '10box-deep-3dp': {
    name: '10BOX-deep-3DP',
    hp: 104,
    material: '3dp',
    panels: [
      { id: 'main-side1', name: 'メイン: サイド1' },
      { id: 'main-side2', name: 'メイン: サイド2' },
      { id: 'main-back1', name: 'メイン: バック1' },
      { id: 'main-bottom1', name: 'メイン: ボトム1' },
      { id: 'main-bottom2', name: 'メイン: ボトム2' },
      { id: 'main-front', name: 'メイン: フロント' },
      { id: 'main-side3', name: 'メイン: サイド3' },
      { id: 'main-side4', name: 'メイン: サイド4' },
      { id: 'lid-side1', name: 'フタ: サイド1' },
      { id: 'lid-back', name: 'フタ: バック' },
      { id: 'lid-top1', name: 'フタ: トップ1' },
      { id: 'lid-top2', name: 'フタ: トップ2' },
      { id: 'lid-front', name: 'フタ: フロント' },
      { id: 'lid-side2', name: 'フタ: サイド2' },
      { id: 'stand-angle1', name: 'スタンド: アングル1' },
      { id: 'stand-angle2', name: 'スタンド: アングル2' },
      { id: 'stand-support1', name: 'スタンド: サポート1' },
      { id: 'stand-support2', name: 'スタンド: サポート2' },
    ],
  },
  'zudo-block-60-open-ACR-A': {
    name: 'zudo-block-60-open-ACR-A',
    hp: 60,
    material: 'acrylic',
    panels: [
      { id: 'side1', name: 'サイド1' },
      { id: 'side2', name: 'サイド2' },
    ],
  },
  'zudo-block-60-open-ACR-B': {
    name: 'zudo-block-60-open-ACR-B',
    hp: 60,
    material: 'acrylic',
    panels: [
      { id: 'side1', name: 'サイド1' },
      { id: 'side2', name: 'サイド2' },
    ],
  },
  'zudo-block-60-open-upgrade-ACR': {
    name: 'zudo-block-60-open-upgrade-ACR',
    hp: 60,
    material: 'acrylic',
    panels: [
      { id: 'back1', name: 'バック1' }, // Position 1: black
      { id: 'back2', name: 'バック2' }, // Position 2: red
      { id: 'bottom1', name: 'ボトム1' }, // Position 3: black
      { id: 'bottom2', name: 'ボトム2' }, // Position 4: red
      { id: 'top1', name: 'トップ1' }, // Position 5: black
      { id: 'top2', name: 'トップ2' }, // Position 6: red
    ],
  },
  'zudo-block-60-open-3DP-A': {
    name: 'zudo-block-60-open-3DP-A',
    hp: 60,
    material: '3dp',
    panels: [
      { id: 'side1', name: 'サイド1' },
      { id: 'side2', name: 'サイド2' },
    ],
  },
  'zudo-block-60-open-3DP-B': {
    name: 'zudo-block-60-open-3DP-B',
    hp: 60,
    material: '3dp',
    panels: [
      { id: 'side1', name: 'サイド1' },
      { id: 'side2', name: 'サイド2' },
    ],
  },
  'zudo-block-60-open-upgrade-3DP': {
    name: 'zudo-block-60-open-upgrade-3DP',
    hp: 60,
    material: '3dp',
    panels: [
      { id: 'back1', name: 'バック1' }, // Position 1: black
      { id: 'back2', name: 'バック2' }, // Position 2: red
      { id: 'bottom1', name: 'ボトム1' }, // Position 3: black
      { id: 'bottom2', name: 'ボトム2' }, // Position 4: red
      { id: 'top1', name: 'トップ1' }, // Position 5: black
      { id: 'top2', name: 'トップ2' }, // Position 6: red
    ],
  },
  'zudo-stand-40': {
    name: 'zudo-stand-40',
    hp: 40,
    material: '3dp',
    panels: [
      { id: 'angle1', name: 'アングル1' },
      { id: 'support1', name: 'サポート1' },
      { id: 'support2', name: 'サポート2' },
      { id: 'angle2', name: 'アングル2' },
    ],
  },
  'zudo-stand-40x2': {
    name: 'zudo-stand-40x2',
    hp: 80,
    material: '3dp',
    panels: [
      { id: 'angle1', name: 'アングル1' },
      { id: 'support1', name: 'サポート1' },
      { id: 'support2', name: 'サポート2' },
      { id: 'angle2', name: 'アングル2' },
    ],
  },
  'zudo-stand-60': {
    name: 'zudo-stand-60',
    hp: 60,
    material: '3dp',
    panels: [
      { id: 'angle1', name: 'アングル1' },
      { id: 'support1', name: 'サポート1' },
      { id: 'support2', name: 'サポート2' },
      { id: 'angle2', name: 'アングル2' },
    ],
  },
  'zudo-stand-60x2': {
    name: 'zudo-stand-60x2',
    hp: 120,
    material: '3dp',
    panels: [
      { id: 'angle1', name: 'アングル1' },
      { id: 'support1', name: 'サポート1' },
      { id: 'support2', name: 'サポート2' },
      { id: 'angle2', name: 'アングル2' },
    ],
  },
  '5box-shallow-3dp': {
    name: '5BOX-shallow-3DP',
    hp: 60,
    material: '3dp',
    panels: [
      { id: 'main-side1', name: 'メイン: サイド1' },
      { id: 'main-side2', name: 'メイン: サイド2' },
      { id: 'main-back1', name: 'メイン: バック' },
      { id: 'main-bottom1', name: 'メイン: ボトム' },
      { id: 'main-bottom2', name: 'メイン: フロント' },
      { id: 'main-front', name: 'メイン: トップ' },
      { id: 'lid-side1', name: 'フタ: サイド1' },
      { id: 'lid-side2', name: 'フタ: サイド2' },
      { id: 'lid-back1', name: 'フタ: バック' },
      { id: 'lid-back2', name: 'フタ: トップ' },
      { id: 'lid-front', name: 'フタ: フロント' },
    ],
  },
  '5box-deep-3dp': {
    name: '5BOX-deep-3DP',
    hp: 60,
    material: '3dp',
    panels: [
      { id: 'main-side1', name: 'メイン: サイド1' },
      { id: 'main-side2', name: 'メイン: サイド2' },
      { id: 'main-back1', name: 'メイン: バック' },
      { id: 'main-bottom1', name: 'メイン: ボトム' },
      { id: 'main-bottom2', name: 'メイン: フロント' },
      { id: 'main-front', name: 'メイン: トップ' },
      { id: 'lid-side1', name: 'フタ: サイド1' },
      { id: 'lid-side2', name: 'フタ: サイド2' },
      { id: 'lid-back1', name: 'フタ: バック' },
      { id: 'lid-back2', name: 'フタ: トップ' },
      { id: 'lid-front', name: 'フタ: フロント' },
    ],
  },
};
