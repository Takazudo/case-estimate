export default function FontDemoPage() {
  ...
  const fontWeights = [
    ...
    { name: 'Black (900)', class: 'font-black' },
  ];

  return (
    <div className="container mx-auto px-4 pt-[200px] pb-[100px]">
      <h1 className="text-3xl font-bold mb-8">Noto Sans Font Weight Demo</h1>

      <div className="space-y-8">
        {fontWeights.map(({ name, class: className }) => (
          <div key={className} className="border-b pb-6">
            <h2 className="text-xl font-semibold mb-3 text-gray-700">{name}</h2>
            <p className={className}>{demoText}</p>
          </div>
        ))}
      </div>
    </div>
